...
typename CollisionChecker::CollisionResult CollisionChecker::CollisionCheck(
    trajectory_math::Boundary boundary) {
  // Ensure that the normal is a unit vector:
  boundary.normal = boundary.normal.normalized();

  // Take the dot product of the trajectory with the unit normal
  // This gives the distance of the trajectory from the plane as a function of
  // time
  double c[5] = {0, 0, 0, 0, 0};
  std::vector<Eigen::Vector3d> trajDerivativeCoeffs = _traj.GetDerivativeCoeffs();
  ...
  for (unsigned i = 0; i < (rootCount + 2); i++) {
    ...
    if (roots[i] < _traj.GetStartTime()) continue;
    if (roots[i] > _traj.GetEndTime()) continue;
...
typename CollisionChecker::CollisionResult CollisionChecker::CollisionCheck(
  ...
  // First check if the start or end point of the trajectory is inside the
  // obstacle
...
typename CollisionChecker::CollisionResult
CollisionChecker::CollisionCheckSection(
    double ts, double tf, std::shared_ptr<trajectory_math::ConvexObj> obstacle,
    double minTimeSection) {
  // First find the position halfway between the start and end time of this
  // segment
  double midTime = (ts + tf) / 2;
  Eigen::Vector3d midpoint = _traj.GetValue(midTime);
  if (obstacle->IsPointInside(midpoint)) {
    // The midpoint is inside the obstacle, so we're infeasible
    return Collision;
  }
  if (tf - ts < minTimeSection) {
    // Our time resolution is too small, just give up (trajectory is likely
    // tangent to obstacle surface)
    return CollisionIndeterminable;
  }

  // Get the plane separating the midpoint and the obstacle
  trajectory_math::Boundary tangentPlane = obstacle->GetTangentPlane(midpoint);

  // Take the dot product of the trajectory with the unit normal of the
  // separating plane. This gives the distance of the trajectory from the plane
  ...
  testPointsLow.push_back(
      ts);  // ts is always the first critical point in testPointsLow
  testPointsHigh.push_back(
      midTime);  // midTime is always the first critical point in testPointsHigh
  for (int i = 0; i < rootCount; i++) {
    if (roots[i] <= ts) {
      ...
      // Because the roots are in ascending order, there are no more roots are
      // on (ts,tf)
      break;
    }
  }
  testPointsLow.push_back(
      midTime);  // midTime is always the last critical point in testPointsLow
  testPointsHigh.push_back(
      tf);  // tf is always the last critical point in testPointsHigh

  // Check testPointsHigh first. We could also check testPointsLow first, but
  // the intuition is that obstacles are more likely to be encountered at later
  // times. We iterate forward in time from midTime to tf.
  for (typename std::vector<double>::iterator it = testPointsHigh.begin() + 1;
    ...
    if ((_traj.GetValue(*it) - tangentPlane.point).dot(tangentPlane.normal) <=
      ...
      CollisionResult resHigh =
          CollisionCheckSection(*(it - 1), tf, obstacle, minTimeSection);
      if (resHigh == NoCollision) {
        // The section from the previous critical point until tf was feasible,
        // meaning that all of the trajectory from midTime to tf does not
        // collide with the obstacle
        break;
      } else {
        // Either a collision was detected between the previous critical point
        // and tf, or the recursion became too deep (i.e. the time resolution
        // too small) and the collision was indeterminable.
        return resHigh;
      }
    }
  }
  // The segment of the trajectory from midTime to tf is feasible, check the
  // segment of the trajectory from midTime to ts (starting from midTime
  // iterating to ts).
