...
RapidTrajectoryGenerator::CheckInputFeasibilitySection(double fminAllowed,
                                                       double fmaxAllowed,
                                                       double wmaxAllowed,
                                                       double t1, double t2,
  ...
  double wBound;
  if (fminSqr > 1e-6)
    wBound = sqrt(jmaxSqr / fminSqr);  // the 1e-6 is a divide-by-zero
                                       // protection
  else
  ...
  // possibly infeasible:
  if (fmin < fminAllowed || fmax > fmaxAllowed ||
      wBound > wmaxAllowed) {  // indeterminate: must check more closely:
    double tHalf = (t1 + t2) / 2;
    InputFeasibilityResult r1 = CheckInputFeasibilitySection(
        fminAllowed, fmaxAllowed, wmaxAllowed, t1, tHalf, minTimeSection);
...
RapidTrajectoryGenerator::InputFeasibilityResult
RapidTrajectoryGenerator::CheckInputFeasibility(double fminAllowed,
                                                double fmaxAllowed,
                                                double wmaxAllowed,
                                                double minTimeSection) {
...
RapidTrajectoryGenerator::CheckPositionFeasibility(Vec3 boundaryPoint,
                                                   Vec3 boundaryNormal) {
...
Vec3 RapidTrajectoryGenerator::GetOmega(double t, double timeStep) const {
  ...
  if (crossProd.GetNorm2())
    return (acos(n0.Dot(n1)) / timeStep) * crossProd.GetUnitVector();
  else
    return Vec3(0, 0, 0);
}
};  // namespace trajectory_generator
