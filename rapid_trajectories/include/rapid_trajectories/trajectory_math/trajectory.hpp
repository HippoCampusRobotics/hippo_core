...
class Trajectory {
 public:
   ...
   */
  Trajectory(std::vector<Eigen::Vector3d> coeffs, double startTime,
             double endTime)
      : _coeffs(coeffs), _startTime(startTime), _endTime(endTime) {
   ...
   * @param endTime The latest time at which the polynomial is defined
   */
  Trajectory(const Eigen::Vector3d &c0, const Eigen::Vector3d &c1,
             const Eigen::Vector3d &c2, const Eigen::Vector3d &c3,
             const Eigen::Vector3d &c4, const Eigen::Vector3d &c5,
   ...
   */
  Eigen::Vector3d GetValue(double t) const {
    assert(t >= _startTime);
    assert(t <= _endTime);
  ...
  std::vector<Eigen::Vector3d> GetCoeffs() { return _coeffs; }
   ...
   */
  std::vector<Eigen::Vector3d> GetDerivativeCoeffs() const {
    std::vector<Eigen::Vector3d> derivCoeffs;
  ...
  inline Eigen::Vector3d operator[](int i) const {
    switch (i) {
      ...
      default:
        ...
        assert(false);
        return Eigen::Vector3d();  // Returns vector of NaNs
    }
  }
