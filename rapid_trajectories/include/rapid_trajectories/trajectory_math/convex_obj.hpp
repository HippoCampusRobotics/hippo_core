...
struct Boundary {
   ...
   */
  Boundary(const Eigen::Vector3d &p, const Eigen::Vector3d &n) {
    point = p;
    normal = n;
  }
  ...
  Eigen::Vector3d normal;  //!< A unit vector normal to the plane
};
...
class ConvexObj {
 public:
   ...
   */
  virtual Boundary GetTangentPlane(const Eigen::Vector3d &testPoint) = 0;
   ...
   */
  virtual bool IsPointInside(const Eigen::Vector3d &testPoint) = 0;
